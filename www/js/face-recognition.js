...
/* ══════════════════════════════════════════
   CALIDAD DE CAPTURA (enrolamiento y detección)
══════════════════════════════════════════ */

// Luminosidad media del frame (0–255) via muestra 64×64
function _luminosidadFrame(video) {
  try {
...
function _evaluarCalidad(det, video) {
  ...
  const lum = _luminosidadFrame(video);
  if (lum < 45)  issues.push('Muy oscuro — acérquese a una fuente de luz.');
  if (lum > 218) issues.push('Exceso de luz — evite la luz solar directa.');
