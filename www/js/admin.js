...
function abrirConfig() {
  ...
  document.getElementById('cfgAlmuerzoMin').value = cfg.almuerzoMin != null ? cfg.almuerzoMin : CONFIG.ALMUERZO_MIN;
...
document.getElementById('btnGuardarConfig').addEventListener('click', () => {
  ...
  const cfg = {
    ...
    almuerzoMin: parseInt(document.getElementById('cfgAlmuerzoMin').value) || 60
  };
...
async function cargarReporte() {
  ...
  const ALMUERZO_MIN = CONFIG.ALMUERZO_MIN || 60;
  tabEl.innerHTML = datos.map(e => {
    const tieneDeuda = (e.minutosDeuda || 0) > 0;
    const almTrab = e.almuerzoTrabajado || false;
    return `<div style="background:var(--wh);border-radius:14px;padding:14px;border:1px solid var(--bd);box-shadow:var(--sh);display:flex;flex-direction:column;gap:10px;">
      ...
      ${tieneDeuda ? `<div style="border-top:1px solid var(--bd);padding-top:10px;display:flex;align-items:center;justify-content:space-between;gap:8px;">
        <div style="font-size:11.5px;color:var(--t1);font-weight:600;line-height:1.4;">
          ${almTrab ? `<span style="color:var(--dg);">✓ Trabajó almuerzo (+${ALMUERZO_MIN} min)</span>` : `<span style="color:var(--t2);">¿Trabajó en almuerzo? (+${ALMUERZO_MIN} min)</span>`}
        </div>
        <button onclick="autorizarAlmuerzo('${e.nombre.replace(/'/g, "\\'")}')" style="background:${almTrab?'var(--dg)':'var(--bg)'};color:${almTrab?'#fff':'var(--t1)'};border:1.5px solid ${almTrab?'var(--dg)':'var(--bd)'};border-radius:8px;padding:6px 12px;font-size:11.5px;font-weight:700;cursor:pointer;white-space:nowrap;">${almTrab ? 'Autorizado' : 'Autorizar'}</button>
      </div>` : ''}
    </div>`;
  }).join('') || '<div style="text-align:center;color:var(--t2);font-size:13px;padding:20px 0;">Sin registros</div>';
}

function autorizarAlmuerzo(nombre) {
  abrirModalSupervisor(
    `Autorizar almuerzo trabajado para <b>${xh(nombre)}</b>. Se descontarán ${CONFIG.ALMUERZO_MIN || 60} minutos de su deuda.`,
    () => {
      const fechaVal = document.getElementById('repFecha').value;
      const ALMUERZO_MIN = CONFIG.ALMUERZO_MIN || 60;
      const payload = {accion:'marcar', nombre, documento:'', finca:CONFIG.FINCA.nombre, tipo:'AlmuerzoTrabajado', cargo:getCargo(nombre), fechaHora:new Date().toISOString(), lat:gpsCoords?gpsCoords.lat:null, lng:gpsCoords?gpsCoords.lng:null, dentroGeocerca:gpsOk, distanciaFacial:null, sinBiometria:true, puntualidad:'ok', minutosDeuda:-ALMUERZO_MIN, fechaDeuda:fechaVal};
      enviar(payload); updColaBadge();
      showToast(`Almuerzo autorizado para ${nombre}`);
      setTimeout(() => cargarReporte(), 800);
    }
  );
}
